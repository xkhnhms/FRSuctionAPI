...
class SealCheckConfig:
    ...
    point_sampler_nums: int = 50   # 单区域点云采样点云数  200  100   80   50  30  40
    ...
    sample_tol: float = 0.003       # 统一采样容差（探针/侧吸盘表面有效距离）, 默认 max(0.005*0.4, 0.0012, 0.005*0.6)=0.003
    ...
    depth_band_m: float = 0.0015        # 基准深度带 1.5mm  (先读这个)
    ...
    side_seal_gpu_min: float = 0.12      # GPU 侧吸盘密封最低分（0~1）, 象限检查防半悬空
    side_seal_n_sec: int = 8             # 侧吸盘密封象限数（柱体角度分箱数，6 表示每 60° 一区）
    ...
    skip_detect_num: int = 10            # skip 检测个数   10  8  7
    ...
    """ 多吸盘场景碰撞参数 """
    side_scene_sample: int = 20000           # 场景碰撞检测时 scene 点云最大采样数（0=全部）
    side_scene_chunk: int = 5000             # 场景碰撞子分块大小（防止 OOM）
